...
class SyncronousTracer(object):
    ...
    def __init__(self, outputs, config, func_name):
        self.processor = TraceProcessor(outputs, config, func_name)
        self.config = config
...
class TraceProcessor(Thread):
    ...
    def init_trace_data(self):
        ...
        self.func_deep = defaultdict(int)
        self.func_deep_max = 1
        self.func_deep['__main__'] = 1
        self.called_func = []
    ...
    def process(self, frame, event, arg, memory=None):
        ...
        if event == 'line':
            ...
            if keep:
                ...
                #     f.write("\n")
                try:
                    statement = inspect.getsourcelines(co)[0][frame.f_lineno - inspect.getsourcelines(co)[1]].strip()
                    if "assert" in statement:
                        # with open("log.txt", "a") as f:
                        #     f.write(statement)
                        #     f.write("\n")
                        if self.called_func and not self.called_func[-1] in self.lcba[full_name]:
                            self.lcba[full_name].append(self.called_func[-1])
        ...
        if event == 'call':
            ...
            if module:
                ...
                if not self.config.include_stdlib \
                        and self.is_module_stdlib(module_path):
                    # with open("log.txt", "a") as f:
                    #     f.write(str(self.config.include_stdlib) + " " + str(self.is_module_stdlib(module_path)) + "\n")
                    keep = False
            ...
            # if "maybe_mangle_lambdas" in func_name:
            #     with open("log.txt", "a") as f:
            #         f.write(str(keep) + " " + str(len(self.call_stack)) + " " + str(self.config.max_depth) + " " + full_name + "\n")
            ...
            if keep:
                if self.call_stack:
                    ...
                    src_func = None
                # with open("log.txt", "a") as f:
                #     f.write("?1? " + str(src_func) + " " + self.func_name + "\n")
                #     f.write("?2? " + str(full_name) + "\n")
                if (not full_name in self.called_func) and self.func_name in src_func:
                    # with open("log.txt", "a") as f:
                    #     f.write("?3? " + str(full_name) + "\n")
                    self.called_func.append(full_name)
                ...
                if not self.func_deep[full_name]:
                    self.func_deep[full_name] = len(self.call_stack)
                    self.func_deep_max = max(self.func_deep_max, self.func_deep[full_name])
    ...
    def stat_group_from_func(self, func, calls):
        ...
        stat_group.call_deep = Stat(
            self.func_deep.get(func, 0), self.func_deep_max
        )
        stat_group.lcba = str(self.lcba.get(func, [])).replace(",", ";")
        return stat_group
