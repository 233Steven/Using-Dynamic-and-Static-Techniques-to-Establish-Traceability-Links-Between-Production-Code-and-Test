...
def run(project_name, project_root):
    ...
    git_pair = util.get_git_info_method(project_root.replace("projects_new", "projects_new_git"))
    # with open("log.txt", "w") as f:
    #     f.write(str(len(git_pair)))
    ...
    for test_file in test_files:
        ...
        for tfunc in function_in_test[test_file]:
            if tfunc.startswith("test_digestauth_401"):
                continue
            test_module = test_file + "@" + tfunc
            ...
            for production_file in production_files:
                ...
                for pfunc in function_in_production[production_file]:
                    ...
                    score["NC"][test_module][production_module] = (pfunc == _tfunc)
                    score["NCC"][test_module][production_module] = (pfunc in _tfunc)
                    ...
                    score["Static LCS-U"][test_module][production_module] = (_lcs / len(production_fqn)) * cc
                    score["Static LCS-B"][test_module][production_module] = (_lcs / max(len(production_fqn), len(test_fqn))) * cc
                    score["Static Leven"][test_module][production_module] = (1 - Levenshtein.distance(production_fqn, test_fqn) / max(len(production_fqn), len(test_fqn))) * cc
                    score["Similarity"][test_module][production_module] = (util.calculate_similarity(code_in_function[test_file][tfunc], code_in_function[production_file][pfunc])) * cc
                    score["Co-ev"][test_module][production_module] = (util.cal_appearance_in_git("@".join([test_file_name, tfunc]), "@".join([production_file_name, pfunc]), git_pair)) * cc
    print("run over")
    # TFIDF
    print("TFIDF....................")
    for test_file in test_files:
                    ...
                    for p_file in production_files:
                        ...
                        for ppfunc in function_in_production[p_file]:
                            ...
                            for item in link_production_list[test_module]:
                                if __production_file in item and ppfunc in item:
                                    cnt1 += 1
                    for t_file in test_files:
                        ...
                        for ttfunc in function_in_test[t_file]:
                            ...
                            for item in link_production_list[tm]:
                               if _production_file in item and pfunc in item:
                                    cnt2 += 1 
                    aaa = 1
                    if cnt1 == 0 or cnt2 == 0:
    ...
    #                 for pfunc in function_in_production[production_file]:
    #                     production_module = production_file + "@" + pfunc
    #                     if score[j][test_module][production_module] > 1:
    #                         print("!!!!!!", j, test_module, production_module)
    # write result
    print("write result................")
    pd = defaultdict(bool)
...
def test_run(project_name, project_root):
    ...
    #     test_file_name = test_file.split("\\")[-1].split(".")[0]
    #     for tfunc in function_in_test[test_file]:
    #         test_module = test_file_name + "." + tfunc
    #         print(test_file, tfunc)
    #         config = Config()
    #         config.trace_filter = GlobbingFilter(include=[project_name+".*","*"+test_file_name+".*","*"+tfunc,"tests.*","test.*"])
    #         # config.trace_filter = GlobbingFilter()
    #         with CallTracer(config, tfunc):
    #             pytest.main(["-q", test_file, "-k", tfunc])
...
if __name__ == "__main__":
    ...
    project_root = "F:\\myPythonProjects\\projects_new\\" + project_name
