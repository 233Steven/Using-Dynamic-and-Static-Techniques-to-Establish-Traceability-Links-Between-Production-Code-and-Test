...
def run(project_name, project_root):
    ...
    git_pair = util.get_git_info_file(project_root.replace("projects_new", "projects_new_git"))
    ...
    for test_file in test_files:
        test_file_name = test_file.split("\\")[-1].split(".")[0]
        if test_file_name == "test_lowlevel":
            continue
        ...
        for production_file in production_files:
            ...
            for key in deep.keys():
                if production_file_name in key:
                    # with open("log.txt", "a") as f:
                    #     f.write(_production_file + " " + pfunc + str(deep[key]) + "\n")
                    cc = pow(0.9, deep[key])
            if cc <= 0 or cc > 1:
                cc = 1
            score["NC"][test_file][production_file] = (production_file_name == _tfile) * cc
            score["NCC"][test_file][production_file] = (production_file_name in _tfile) *cc
